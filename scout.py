...
Usage:
  ...
  python scout.py --mode opportunity               # V2 Stage 0, Stage 3, Stage 4, Stage 5, Stage 6, then Stage 7
  ...
  python scout.py --mode opportunity --from 5      # V2 Stage 5 then Stage 6 using existing Stage 4 survivors
  python scout.py --mode opportunity --from 6      # V2 Stage 6 then Stage 7 using existing Stage 5 evidence
  python scout.py --mode opportunity --from 7      # V2 Stage 7 using existing Stage 6 output
...
from stage6_opportunity import run_opportunity_stage6
from stage7_opportunity import generate_opportunity_report
...
def main():
    ...
    if args.mode == "opportunity":
        opportunity_stages = [
            ...
            (6, "Opportunity semantic judgement", run_opportunity_stage6),
            (7, "Opportunity HTML report", generate_opportunity_report),
        ]
        for stage_num, name, fn in opportunity_stages:
            ...
            try:
                ...
                sys.exit(1)
            if stage_num == 7:
                print("\nV2 Stage 7 report generated.")
                print("Open data/opportunity_report.html in a browser.")
        return
